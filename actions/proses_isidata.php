...
// Jika ada error, kembali ke halaman isi data
if (!empty($errors)) {
    ...
    header('Location: ../public/isidata.php' . $fromParam);
    exit;
}
...
function saveDraftPaymentData(PDO $pdo, int $idBooking, string $nama, string $hp, string $alamat, float $totalHarga): void
{
    $existingStmt = $pdo->prepare('SELECT id_pembayaran FROM pembayaran WHERE id_booking = ? LIMIT 1');
    $existingStmt->execute([$idBooking]);
    $idPembayaran = $existingStmt->fetchColumn();

    $columns = ['id_booking', 'jumlah_bayar', 'metode_bayar', 'status_verifikasi'];
    $values = [$idBooking, $totalHarga, 'transfer', 'pending'];
    ...
    if ($idPembayaran) {
        ...
        foreach ($columns as $index => $column) {
            if ($column === 'id_booking') {
                continue;
            }
            $set[] = "`$column` = ?";
            $params[] = $values[$index];
        }
        $params[] = (int) $idPembayaran;
        $stmt = $pdo->prepare('UPDATE pembayaran SET ' . implode(', ', $set) . ' WHERE id_pembayaran = ?');
        $stmt->execute($params);
        return;
    }

    $placeholders = implode(', ', array_fill(0, count($columns), '?'));
    $stmt = $pdo->prepare('INSERT INTO pembayaran (' . implode(', ', $columns) . ') VALUES (' . $placeholders . ')');
    $stmt->execute($values);
}
...
try {
    ...
    $totalHarga = (float) ($draft['total'] ?? $draft['harga'] ?? 0);
    if (empty($draft['id_booking'])) {
        ...
        saveDraftPaymentData($pdo, $idBooking, $nama, $hp, $alamat, $totalHarga);
...
unset($_SESSION['draft_booking'], $_SESSION['checkout_booking'], $_SESSION['pembayaran'], $_SESSION['form_data']);
$_SESSION['success_message'] = 'Booking berhasil dikirim. Silakan cek histori untuk melihat status dan link pembayaran setelah admin mengonfirmasi.';

header('Location: ../public/riwayat_pesanan.php?booking_submitted=1');
exit;
?>

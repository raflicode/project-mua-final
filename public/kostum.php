...
.card-custom{ border:none; border-radius:20px; overflow:hidden; background:#fff; box-shadow:0 5px 15px rgba(0,0,0,.12); transition:.3s; height:100%; cursor:pointer; }
.card-custom:hover{ transform:translateY(-5px); }
.card-body{ display:flex; flex-direction:column; height:100%; }
.img-paket-wrap{ width:100%; aspect-ratio:4/5; overflow:hidden; border-radius:14px; margin-bottom:14px; background:#f3f3f3; }
...
.modal-kostum .modal-header{ background:#a88656; border:none; padding:16px 18px; flex-direction:column; align-items:stretch; }
.modal-level2-bar{ display:flex; align-items:center; justify-content:center; gap:8px; margin-top:12px; padding-top:10px; border-top:1px solid rgba(255,255,255,.25); }
...
@media(max-width:576px){
    ...
    .modal-kostum .modal-header{ padding:11px 12px; }
    .modal-level2-bar{ margin-top:8px; padding-top:8px; gap:6px; }
...
<button type="button" id="modalCartBtn" class="btn btn-dark flex-grow-1"><i class="bi bi-cart3"></i> Keranjang</button>
...
const kostumData = <?= json_encode($kostumData, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>;
        ...
        <div class="col-12 col-md-6 col-lg-3">
            <div class="card card-custom p-3" onclick="bukaModal(${i})">
                <div class="card-body">
                    ...
                    <div class="img-paket-wrap"><img src="${first.foto}" alt="${first.nama}" onerror="this.src='https://placehold.co/400x500?text=Foto'"></div>
                    <p class="fw-semibold mb-1">Include :</p>
                    <ol class="include-ol">${first.include.map(x => `<li>${x}</li>`).join('')}</ol>
...
function renderModal(){
    ...
    document.getElementById('variantDots').innerHTML = kostum.variasi.map((_, i) => `<div class="variant-dot ${i === idxVariasi ? 'active' : ''}"></div>`).join('');
}
...
document.getElementById('modalCartBtn').addEventListener('click', () => {
    ...
    const kostum = kostumData[idxKostum];
    const varian = kostum.variasi[idxVariasi];
    addToCart(`${kostum.jenis} - ${varian.nama}`, 'kostum', hargaKeAngka(varian.harga), varian.foto, varian.id);
});
...
document.getElementById('modalBookingBtn').addEventListener('click', () => {
    ...
    const kostum = kostumData[idxKostum];
    const varian = kostum.variasi[idxVariasi];
    window.location.href = `booking.php?from=kostum&id=${encodeURIComponent(varian.id)}&layanan=${encodeURIComponent(`${kostum.jenis} - ${varian.nama}`)}&harga=${hargaKeAngka(varian.harga)}`;
});
